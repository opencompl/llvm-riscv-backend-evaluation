...
/*
 * Kernel under test: libdivide_s32_recover as lowered by the pipeline
 ...
 */
extern void bench_kernel(const struct libdivide_s32_t *denom, int32_t *out);
 ...
 */
static int32_t divisors[NUM_DIVISORS];
static struct libdivide_s32_t magics[NUM_DIVISORS];
static int32_t recovered[NUM_DIVISORS];
...
init_divisors(void)
{
    ...
    for (int i = 0; i < NUM_DIVISORS; i++) {
        ...
        int32_t d = (int32_t)(x >> 33); /* positive 31-bit value */
        if (x & 1) {
...
main(void)
{
    ...
    for (int i = 0; i < NUM_DIVISORS; i++) {
        magics[i] = libdivide_s32_gen(divisors[i]);
    }
